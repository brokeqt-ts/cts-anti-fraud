...
export async function notifyOwnerAndAdmins(
  ...
  if (settingsKey) {
    ...
    // 3. Override severity from settings
    const effectiveParams = { ...params, severity: setting.severity };
  ...
  // No settings key — send unconditionally (legacy behavior)
  const notifiedIds = new Set<string>();

  if (ownerUserId) {
    await createNotification(pool, { ...params, userId: ownerUserId });
    notifiedIds.add(ownerUserId);
  }
  ...
  for (const admin of admins.rows) {
    const adminId = admin['id'] as string;
    if (!notifiedIds.has(adminId)) {
      await createNotification(pool, { ...params, userId: adminId });
      notifiedIds.add(adminId);
    }
  }
