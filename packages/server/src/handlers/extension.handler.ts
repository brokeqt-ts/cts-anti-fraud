...
async function streamExtensionZip(
  ...
  const result = await pool.query(
    'SELECT api_key, name, adspower_api_key, adspower_api_url FROM users WHERE id = $1 AND is_active = true',
    [userId],
  );

  const row = result.rows[0] as { api_key?: string; name?: string; adspower_api_key?: string; adspower_api_url?: string } | undefined;
  if (!row) {
  ...
  const adspowerUrl = row.adspower_api_url ?? '';

  addDirectoryToArchive(archive, extDistPath, 'cts-extension', apiKey, serverUrl, adspowerKey, adspowerUrl);
...
function addDirectoryToArchive(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      addDirectoryToArchive(archive, fullPath, archivePath, apiKey, serverUrl, adspowerKey, adspowerUrl);
      ...
      const replaced = content
        ...
        .replaceAll(ADSPOWER_URL_PLACEHOLDER, adspowerUrl);
