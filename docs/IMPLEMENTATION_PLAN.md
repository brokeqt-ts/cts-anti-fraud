...
Модальное окно поиска по всему приложению: аккаунты, домены, баны, уведомления.
Как в Linear/Notion — keyboard-first навигация.

- Бэкенд: `GET /api/v1/search?q=...` — поиск по accounts, domains, bans
- Фронтенд: `CommandPalette` компонент, hotkey Ctrl+K / Cmd+K
- Результаты группированы по типу, клик → переход на страницу

#### UX-2: Фильтр по датам на всех страницах
**Оценка:** 1 день
**Зависимости:** нет

Компонент DateRangePicker: "Сегодня / 7д / 30д / 90д / Произвольный".
Добавить на: баны, уведомления, аналитику, activity feed.
Бэкенд: параметры `from` / `to` на существующих эндпоинтах.
...
**Оценка:** 2 дня
**Зависимости:** миграция — таблица account_tags

Теги (`#gambling-eu`, `#nutra-tier1`), группы по байеру.
Фильтр "мои аккаунты" на странице accounts.
Как в AdsPower — профили по папкам.

#### UX-7: Ban Chain граф-визуализация
**Оценка:** 2-3 дня
**Зависимости:** react-force-graph или d3

Интерактивный граф: узлы = аккаунты, рёбра = общие домены/BIN/прокси.
Вместо таблицы — видно кластеры связанных аккаунтов.
...
Хронологическая лента: создание → кампания → смена домена → warning → бан.
Единый timeline view на странице аккаунта.
...
Кто что сделал: записал бан, поменял настройки, скачал расширение.
Таблица audit_log + страница /admin/audit.
